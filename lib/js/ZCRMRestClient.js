...
const localUtil = require("./local/local_util");
...
ZCRMRestClient.initialize = function (configJSON) {
  ...
  data.client_id =
    data.client_id ||
    config.get("zoho.crm.client_id") ||
    config.get("zoho.crm.clientid") ||
    null;
  data.client_secret =
    data.client_secret ||
    config.get("zoho.crm.client_secret") ||
    config.get("zoho.crm.clientsecret") ||
    null;
  data.redirect_url =
    data.redirect_url ||
    config.get("zoho.crm.redirect_url") ||
    config.get("zoho.crm.redirecturl") ||
    null;
  data.refresh_token =
    data.refresh_token ||
    config.get("zoho.crm.refresh_token") ||
    config.get("zoho.crm.refreshtoken") ||
    null;
  data.base_url =
    data.base_url ||
    config.get("crm.api.base_url") ||
    config.get("crm.api.url") ||
    null;
  data.user_identifier =
    data.user_identifier || config.get("crm.api.user_identifier") || null;
  data.mysql_module =
    data.mysql_module ||
    config.get("crm.api.mysql_module") ||
    config.get("crm.api.tokenmanagement") ||
    null;
  data.mysql_username =
    data.mysql_username || config.get("mysql.username") || null;
  data.mysql_password =
    data.mysql_password || config.get("mysql.password") || null;
  ...
  data.client_secret =
    data.client_secret || process.env.ZCRM__client_secret || null;
  data.redirect_url =
    data.redirect_url || process.env.ZCRM__redirect_url || null;
  data.refresh_token =
    data.refresh_token || process.env.ZCRM__refresh_token || null;
  data.base_url = data.base_url || process.env.ZCRM__base_url || null;
  data.user_identifier =
    data.user_identifier || process.env.ZCRM__user_identifier || null;
  data.mysql_module =
    data.mysql_module || process.env.ZCRM__mysql_module || null;
  data.mysql_username =
    data.mysql_username || process.env.ZCRM__mysql_username || null;
  data.mysql_password =
    data.mysql_password || process.env.ZCRM__mysql_password || null;
  ...
  if (missingValues.length > 0) {
    throw new Error(
      `The following configuration values were not given values: ${missingValues.join(
        ", ",
      )}`,
    );
  }
  ...
  return new Promise(function (resolve, reject) {
    ...
    OAuth.generateTokens(api_url).then(function (response) {
      if (response.statusCode != 200) {
        throw new Error(
          "Problem occurred while generating access token from grant token. Response : " +
            JSON.stringify(response),
        );
      }

      // const mysql_util = require(ZCRMRestClient.getMySQLModule());
      const mysql_util = localUtil;
      ...
      if (resultObj.access_token) {
        ...
        throw new Error(
          "Problem occurred while generating access token and refresh token from grant token.Response : " +
            JSON.stringify(response),
        );
      }
    });
...
ZCRMRestClient.generateAuthTokenfromRefreshToken = function (
  ...
  return new Promise(function (resolve, reject) {
    ...
    OAuth.generateTokens(api_url).then(function (response) {
      if (response.statusCode != 200) {
        throw new Error(
          "Problem occurred while generating access token from refresh token . Response : " +
            JSON.stringify(response),
        );
      }
      const mysql_util = localUtil;
